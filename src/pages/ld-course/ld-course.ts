...
@IonicPage()
@Component({
  templateUrl: 'ld-course.html',
  selector: 'ld-course'
})
export class LdCoursePage implements OnInit {
  selectedItem: any;
  content: any;
  course: any;
  lessons: any;
  ...
  getCourse( id ) {
    ...
    this.ldProvider.get( 'courses/' + id, '1' ).then( item => {
      
      this.course = item
      console.log( this.course.course_materials )
      this.getLessons( id )
      this.addListeners()
    }).catch( err => {
      console.warn(err)
  ...
  getLessons( id ) {
    ...
    this.ldProvider.get( 'courses/' + id + '/lessons', '1' ).then( lessons => {
      console.log( lessons )
  ...
  ionViewWillEnter() {
    ...
    if(this.selectedItem && this.selectedItem.link)
      this.ga.trackScreenView('LdCoursePage', 'link/' + this.selectedItem.link);
